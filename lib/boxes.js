...
const index_copy = (mode, mode_length) => (in_fh, out_fh, box, delta) => {
  ...
  const header = jspack.unpack(">I", fh.read(4))
  const values = jspack.unpack(">I", fh.read(4))

  const new_contents = []
  new_contents.push(jspack.pack(">I", header))
  new_contents.push(jspack.pack(">I", values))
  ...
  for (let i = 0; i < values; i += 1) {
    ...
    new_contents.push(jspack.pack(mode, content))
  }
  ...
  */
  ...
  save (in_fh, out_fh, delta) {
    ...
    if (this.header_size === 16) {
      ...
      out_fh.write(jspack.pack(">I", this.size())) // this does not output write thing
      out_fh.write(new Buffer(this.name))
    }
...
class container_box extends box {
  ...
  save (in_fh, out_fh, delta) {
    if (this.header_size === 16) {
      ...
      out_fh.write(jspack.pack(">I", this.size())) // not outputing/writing correct value
      out_fh.write(new Buffer(this.name))
    }
